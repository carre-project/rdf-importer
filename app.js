...
function sendEmail(status,result, job) {
  ...
  sendgrid.sendMail({
    ...
    html:`
        ...
        <h3>Job #${job.id} in <span style="text-decoration: underline;">${job.deployment}</span> vm for /<span style="text-decoration: underline;">${job.graph}</span> resulted in <span style="text-decoration: underline;">${job.status}</span> on ${new Date().toString().split(' (')[0]}</h3>
